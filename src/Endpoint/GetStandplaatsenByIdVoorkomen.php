...
/**
 * Class GetStandplaatsenByIdVoorkomen
 *
 * @package BureauVierkant\Basisregistratie\Endpoint
 */
class GetStandplaatsenByIdVoorkomen extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    protected $id;
    /**
     * 
    ...
    {
        return str_replace(array('{id}'), array($this->id), '/standplaatsen/{id}/voorkomens');
    }
     ...
     *
     * @return null|\BureauVierkant\Basisregistratie\Model\StandplaatsVoorkomensCollection
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'BureauVierkant\\Basisregistratie\\Model\\StandplaatsVoorkomensCollection', 'json');
        }
        if (403 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetStandplaatsenByIdVoorkomenForbiddenException();
        }
        if (404 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetStandplaatsenByIdVoorkomenNotFoundException();
        }
    }
}