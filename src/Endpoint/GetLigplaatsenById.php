...
/**
 * Class GetLigplaatsenById
 *
 * @package BureauVierkant\Basisregistratie\Endpoint
 */
class GetLigplaatsenById extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    protected $id;
    ...
    {
        return str_replace(array('{id}'), array($this->id), '/ligplaatsen/{id}');
    }
     ...
     *
     * @return null|\BureauVierkant\Basisregistratie\Model\Ligplaats
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'BureauVierkant\\Basisregistratie\\Model\\Ligplaats', 'json');
        }
        if (403 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetLigplaatsenByIdForbiddenException();
        }
        if (404 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetLigplaatsenByIdNotFoundException();
        }
    }
}