...
/**
 * Class PostVerblijfsobjecten
 *
 * @package BureauVierkant\Basisregistratie\Endpoint
 */
class PostVerblijfsobjecten extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    ...
    protected function getQueryOptionsResolver() : \Symfony\Component\OptionsResolver\OptionsResolver
    {
        ...
        $optionsResolver->setDefined(array('pandrelatering', 'geldigOp', 'page'));
        $optionsResolver->setRequired(array());
        $optionsResolver->setDefaults(array('page' => 1));
        $optionsResolver->setAllowedTypes('pandrelatering', array('string'));
     ...
     *
     * @return null|\BureauVierkant\Basisregistratie\Model\VerblijfsobjectCollection
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'BureauVierkant\\Basisregistratie\\Model\\VerblijfsobjectCollection', 'json');
        }
        if (400 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\PostVerblijfsobjectenBadRequestException();
        }
        if (403 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\PostVerblijfsobjectenForbiddenException();
        }
    }
}