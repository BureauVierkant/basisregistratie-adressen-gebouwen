...
/**
 * Class GetLigplaatsenByIdVoorkomen
 *
 * @package BureauVierkant\Basisregistratie\Endpoint
 */
class GetLigplaatsenByIdVoorkomen extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    protected $id;
    /**
     * 
    ...
    {
        return str_replace(array('{id}'), array($this->id), '/ligplaatsen/{id}/voorkomens');
    }
     ...
     *
     * @return null|\BureauVierkant\Basisregistratie\Model\LigplaatsVoorkomensCollection
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'BureauVierkant\\Basisregistratie\\Model\\LigplaatsVoorkomensCollection', 'json');
        }
        if (403 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetLigplaatsenByIdVoorkomenForbiddenException();
        }
        if (404 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetLigplaatsenByIdVoorkomenNotFoundException();
        }
    }
}