...
/**
 * Class GetOpenbareRuimteById
 *
 * @package BureauVierkant\Basisregistratie\Endpoint
 */
class GetOpenbareRuimteById extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    protected $id;
    ...
    {
        return str_replace(array('{id}'), array($this->id), '/openbare-ruimtes/{id}');
    }
     ...
     *
     * @return null|\BureauVierkant\Basisregistratie\Model\Openbareruimte
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'BureauVierkant\\Basisregistratie\\Model\\Openbareruimte', 'json');
        }
        if (403 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetOpenbareRuimteByIdForbiddenException();
        }
        if (404 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetOpenbareRuimteByIdNotFoundException();
        }
    }
}