...
/**
 * Class GetPandenByIdVoorkomen
 *
 * @package BureauVierkant\Basisregistratie\Endpoint
 */
class GetPandenByIdVoorkomen extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    protected $id;
    /**
     * 
    ...
    {
        return str_replace(array('{id}'), array($this->id), '/panden/{id}/voorkomens');
    }
     ...
     *
     * @return null|\BureauVierkant\Basisregistratie\Model\PandVoorkomensCollection
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'BureauVierkant\\Basisregistratie\\Model\\PandVoorkomensCollection', 'json');
        }
        if (403 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetPandenByIdVoorkomenForbiddenException();
        }
        if (404 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetPandenByIdVoorkomenNotFoundException();
        }
    }
}