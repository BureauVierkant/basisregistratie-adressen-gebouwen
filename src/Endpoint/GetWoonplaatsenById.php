...
/**
 * Class GetWoonplaatsenById
 *
 * @package BureauVierkant\Basisregistratie\Endpoint
 */
class GetWoonplaatsenById extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    protected $id;
    ...
    {
        return str_replace(array('{id}'), array($this->id), '/woonplaatsen/{id}');
    }
     ...
     *
     * @return null|\BureauVierkant\Basisregistratie\Model\Woonplaats
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'BureauVierkant\\Basisregistratie\\Model\\Woonplaats', 'json');
        }
        if (403 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetWoonplaatsenByIdForbiddenException();
        }
        if (404 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetWoonplaatsenByIdNotFoundException();
        }
    }
}