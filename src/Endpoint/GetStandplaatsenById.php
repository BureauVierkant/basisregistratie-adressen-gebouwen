...
/**
 * Class GetStandplaatsenById
 *
 * @package BureauVierkant\Basisregistratie\Endpoint
 */
class GetStandplaatsenById extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    protected $id;
    ...
    {
        return str_replace(array('{id}'), array($this->id), '/standplaatsen/{id}');
    }
     ...
     *
     * @return null|\BureauVierkant\Basisregistratie\Model\Standplaats
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'BureauVierkant\\Basisregistratie\\Model\\Standplaats', 'json');
        }
        if (403 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetStandplaatsenByIdForbiddenException();
        }
        if (404 === $status) {
            throw new \BureauVierkant\Basisregistratie\Exception\GetStandplaatsenByIdNotFoundException();
        }
    }
}